(function() {
  ...
  var send = function(options) {
    ...
    xhr = _.extend(xhr, options)
    if (options.upload) {
      xhr.upload = _.extend(xhr.upload, options.upload)
    }
    ...
    var callback = function(err) {
      ...
      if (err) return options.callback(err, xhr);
      if (xhr.status === 0) return options.callback(new Error('network error'), xhr);
      ...
      if (options.json) {
        try {
          ...
          return options.callback(err, xhr);
        }
      }
