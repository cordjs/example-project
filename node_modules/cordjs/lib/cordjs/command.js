...
var copyFile, exec, exists, fs, fsExtra, outputPath, path, printLine, printWarn, publicDir, removeSource, sources, spawn, syncFile, syncFiles, targetDir, timeLog, unwatchDir, util, wait, walk, watchDir, watchFile, _ref;
    ...
    return exec("mkdir -p " + (path.join(targetDir)), function() {
      return fs.realpath(publicDir, function(err, source) {
        return syncFiles(source, path.normalize(source), function() {
          return exec("coffee -bc -o " + targetDir + " " + publicDir);
        });
      });
    });
  });
...
syncFiles = function(source, base, callback) {
  ...
  return fs.stat(source, function(err, stats) {
    ...
    walker.on('file', function(root, stat, next) {
      source = path.join(root, stat.name);
      return syncFile(source, base, next);
    });
